...
public class Main {
    ...
    public static void topTenEconomicalBowlersIn2015() throws Exception{
        ...
        Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/mydb", "ishan", "Ishan2712#" );
    ...
    public static void numberOfMatchesWonByAllTeams() throws Exception{
        System.out.printf("%-40s %-10s%n","Team","Matches Won\n");
        Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/mydb", "ishan", "Ishan2712#" );
        String query = "select winner, count(*) as wins from matches where winner is not null group by winner order by wins desc";
        Statement state = con.createStatement();
        ResultSet result = state.executeQuery(query);
        while(result.next()) {
            System.out.printf("%-40s %-10d%n",result.getString("winner"),result.getInt("wins"));
        }
        System.out.println();
        System.out.println();
        System.out.println();
